...
class ApplicationStorage:
    ...
    def __init__(self, config: Optional[ApplicationConfig] = None):
        ...
        if not self.config.pg_vector_url:
            raise ValueError("pg_vector_url must be configured in settings or provided directly")
        if not self.config.openai_api_key:
            raise ValueError("openai_api_key must be configured in settings or provided directly")
    ...
    def create_application(self, name: str, json_schema: Dict[str, Any], purpose: str) -> str:
        ...
        self.vector_store.add_documents(
            "applications",
            [Document(
                page_content=json.dumps(metadata.dict()),
                metadata={
                    "type": "application",
                    "app_id": app_id
                }
