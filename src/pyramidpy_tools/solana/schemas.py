...
class GetBalanceRequest(BaseModel):
    token_address: Optional[str] = Field(
        None, description="Optional SPL token mint address. If not provided, returns SOL balance"
    )
...
class TradeRequest(BaseModel):
    ...
    percentage: Optional[int] = Field(None, description="Percentage of balance to trade (for sells)")
...
class MoonshotTradeRequest(BaseModel):
    ...
    collateral_amount: Optional[float] = Field(0.01, description="Amount of SOL to spend")
