...
async def get_priority_fees(connection: AsyncClient) -> dict:
    ...
    try:
        ...
        sorted_fees = sorted(
            f["prioritizationFee"]
            for f in priority_fees_resp
            if "prioritizationFee" in f
        )
        ...
        median_fee = (
            ...
            if len(sorted_fees) % 2 == 0
            else sorted_fees[mid]
        )
...
async def send_tx(
    agent: SolanaAgentKit, tx: Transaction, other_keypairs: list[Keypair] = None
) -> str:
    """
    ...
    try:
        ...
        if fees.get("instructions"):
            tx.add(
                fees["instructions"]["medium"]
            )  # Add the medium fee level by default
...
async def sign_and_send_transaction(
    agent: SolanaAgentKit, tx: VersionedTransaction, mint_keypair: Keypair
) -> str:
    """
