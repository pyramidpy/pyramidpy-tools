...
def get_curve_state(agent: SolanaAgentKit, mint_str: str):
    try:
        ...
        MOONSHOT_PROGRAM = Pubkey.from_string(
            "MoonCVVNZFSYkqNXP6bxHLPL6QQJiMagDL3qcqUQTrG"
        )
        SEED = "token".encode()

        curve_account, _ = Pubkey.find_program_address(
            [SEED, bytes(mint)], MOONSHOT_PROGRAM
        )
...
def get_collateral_amount_by_tokens(
    mint_str: str, token_amount: float, direction: TradeDirection
):
...
def get_collateral_price(tokens_amount, curve_position):
    ...
    n = (Decimal(tokens_amount) / TOKEN_DECIMALS).quantize(
        Decimal("1.00000000000000000000")
    )
    m = (Decimal(curve_position) / TOKEN_DECIMALS).quantize(
        Decimal("1.00000000000000000000")
    )

    result = (
        (Decimal("0.5") * COEF_A * n * (Decimal("2") * m + n) + coef_b * n)
        * COLLATERAL_DECIMALS
    ).quantize(Decimal("1.00000000000000000000"))
...
def get_tokens_by_collateral_amount(
    mint_str: str, collateral_amount: float, direction: TradeDirection
):
    try:
        collateral_amount = int(collateral_amount * LAMPORTS_PER_SOL)
        curve_state = get_curve_state(mint_str)
        ...
        tokens = get_tokens_nr_from_collateral(
            collateral_amount, curve_position, direction
        )
...
def get_tokens_nr_from_collateral(collateral_amount, curve_position, direction):
    ...
    y = (Decimal(collateral_amount) / COLLATERAL_DECIMALS).quantize(
        Decimal("1.00000000000000000000")
    )
    m = (Decimal(curve_position) / TOKEN_DECIMALS).quantize(
        Decimal("1.00000000000000000000")
    ...
    discriminant = (b ** Decimal(2) - Decimal(4) * COEF_A * c).quantize(
        Decimal("1.00000000000000000000")
    )
