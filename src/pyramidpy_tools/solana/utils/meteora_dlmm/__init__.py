...
async def create_customizable_permissionless_lb_pair(
    ...
    program_id = (
        opt.get("program_id") if opt else LBCLMM_PROGRAM_IDS[opt.get("cluster")]
    )
    program = Program(idl_data, program_id, provider)

    lb_pair, _ = derive_customizable_permissionless_lb_pair(
        token_x, token_y, program.program_id
    )
    ...
    transaction = (
        await program.methods.initialize_customizable_permissionless_lb_pair(ix_data)
        .accounts(
            {
                "lbPair": lb_pair,
