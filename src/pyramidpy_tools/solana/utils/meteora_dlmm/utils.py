...
def sort_token_mints(
    token_x: PublicKey, token_y: PublicKey
) -> Tuple[PublicKey, PublicKey]:
    """Sorts two public keys and returns the minimum and maximum."""
    min_key, max_key = (
        (token_y, token_x) if bytes(token_x) > bytes(token_y) else (token_x, token_y)
    )
...
def derive_reserve(
    token: PublicKey, lb_pair: PublicKey, program_id: PublicKey
) -> Tuple[PublicKey, int]:
...
def derive_bin_array(
    lb_pair: PublicKey, index: int, program_id: PublicKey
) -> Tuple[PublicKey, int]:
...
def is_overflow_default_bin_array_bitmap(bin_array_index: int) -> bool:
    ...
    return (
        bin_array_index > max_bin_array_index or bin_array_index < min_bin_array_index
    )


def derive_bin_array_bitmap_extension(
    lb_pair: PublicKey, program_id: PublicKey
) -> Tuple[PublicKey, int]:
