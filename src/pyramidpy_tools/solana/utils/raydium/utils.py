...
from .layouts import LIQUIDITY_STATE_LAYOUT_V4, MARKET_STATE_LAYOUT_V3, SWAP_LAYOUT
...
def get_pair_address_from_rpc(client: AsyncClient, token_address: str) -> str:
    ...
    RAYDIUM_PROGRAM_ID = PublicKey.from_string(
...
def make_swap_instruction(
    ...
    try:
        ...
        data = SWAP_LAYOUT.build(
            dict(instruction=9, amount_in=amount_in, min_amount_out=minimum_amount_out)
        )
...
def get_token_balance(agent: SolanaAgentKit, mint_str: str) -> float | None:
    try:
        ...
        response = agent.connection.get_account_info_json_parsed(
            agent.wallet_address, TokenAccountOpts(mint=mint), commitment=Processed
        )
        accounts = response.value
        if accounts:
...
def get_token_reserves(client: AsyncClient, pool_keys: PoolKeys) -> tuple:
    try:
        ...
        balances_response = client.get_multiple_accounts_json_parsed(
            [base_vault, quote_vault], Processed
        )
        ...
        token_account_balance = token_account.data.parsed["info"]["tokenAmount"][
            "uiAmount"
        ]
...
def sol_for_tokens(
    spend_sol_amount, base_vault_balance, quote_vault_balance, swap_fee=0.25
):
...
def tokens_for_sol(
    sell_token_amount, base_vault_balance, quote_vault_balance, swap_fee=0.25
):
