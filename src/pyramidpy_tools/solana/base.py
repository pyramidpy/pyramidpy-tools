...
class SolanaAPI:
    def __init__(
        ...
        self.rpc_url = (
            rpc_url or settings.tool_provider.solana_rpc_url.get_secret_value()
        )
    ...
    async def get_balance(self, token_address: Optional[str] = None) -> BalanceResponse:
        ...
        try:
            if not token_address:
                ...
                return BalanceResponse(
                    success=True, balance=response.value / LAMPORTS_PER_SOL
                )

            token_pubkey = Pubkey.from_string(token_address)
            token = AsyncToken(
                self.connection, token_pubkey, TOKEN_PROGRAM_ID, self.keypair
            )
            ...
            return BalanceResponse(success=False, error=str(e), balance=0)

    async def deploy_token(
        self, name: str, symbol: str, decimals: int, initial_supply: float
    ) -> TokenDeployResponse:
        ...
        try:
            ...
            mint_tx = await token.mint_to(
                token_account, self.wallet_address, int(initial_supply * (10**decimals))
            )
    ...
    async def burn_and_close(
        self, token_address: str, token_account: str
    ) -> BurnAndCloseResponse:
        ...
        try:
            ...
            return BurnAndCloseResponse(
                success=True, transaction_signature=close_tx.value
            )

        except Exception as e:
            logger.error(f"Failed to burn and close: {str(e)}")
            return BurnAndCloseResponse(
                success=False, error=str(e), transaction_signature=""
            )
    ...
    async def trade_with_moonshot(
        ...
        try:
            ...
            return MoonshotTradeResponse(
                success=False, error=str(e), transaction_signature="", amount=0
            )
    ...
    async def create_meteora_pool(
        ...
        try:
            ...
            return CreateDLMMPoolResponse(
                success=False, error=str(e), pool_address="", transaction_signature=""
            )

    async def get_token_price(
        self, token_address: str, vs_currency: str = "usd"
    ) -> PriceResponse:
    ...
    async def get_token_data(self, token_address: str) -> TokenDataResponse:
        """Get detailed token data."""
        from .utils.token_data import get_token_data

        try:
            data = await get_token_data(token_address)
            return TokenDataResponse(success=True, **data)
        except Exception as e:
