...
class SolanaTransferHelper:
    ...
    async def transfer_native_sol(
        agent: SolanaAgentKit, to: Pubkey, amount: float
    ) -> str:
        """
        Transfer native SOL.
    ...
    async def transfer_spl_tokens(
        ...
        spl_client = AsyncToken(
            rpc_client, spl_token, TOKEN_PROGRAM_ID, agent.wallet_address
        )
...
class TokenTransferManager:
    ...
    async def execute_transfer(
        self, to: Pubkey, amount: float, mint: Optional[Pubkey] = None
    ) -> TransferResult:
        """
        ...
        try:
            if mint:
                signature = await SolanaTransferHelper.transfer_spl_tokens(
                    self.agent, to, amount, mint
                )
                token_identifier = str(mint)
            else:
                signature = await SolanaTransferHelper.transfer_native_sol(
                    self.agent, to, amount
                )
    ...
    async def verify_transfer(self, transfer_result: TransferResult) -> bool:
        ...
        try:
            transaction_info = await self.agent.connection.get_transaction(
                transfer_result.signature, commitment=Confirmed
            )
