...
from agentipy.utils.moonshot.curve import (
    TradeDirection,
...
from solders.compute_budget import (
    set_compute_unit_limit,  # type: ignore
...
class MoonshotManager:
    ...
    def buy(
        ...
        mint_str: str,
        collateral_amount: float = 0.01,
        slippage_bps: int = 500,
    ):
        try:
            client = Client(agent.rpc_url)
            amount = get_tokens_by_collateral_amount(
                mint_str, collateral_amount, TradeDirection.BUY
            )

            collateral_amount = int(collateral_amount * LAMPORTS_PER_SOL)

            print(
                f"Collateral Amount (in lamports): {collateral_amount}, Amount: {amount}, Slippage (bps): {slippage_bps}"
            )
            ...
            try:
                account_data = agent.connection.get_token_accounts_by_owner(
                    SENDER, TokenAccountOpts(MINT)
                )
            ...
            keys = [
                ...
                AccountMeta(
                    pubkey=SENDER_TOKEN_ACCOUNT, is_signer=False, is_writable=True
                ),
                AccountMeta(pubkey=CURVE_ACCOUNT, is_signer=False, is_writable=True),
                AccountMeta(
                    pubkey=CURVE_TOKEN_ACCOUNT, is_signer=False, is_writable=True
                ),
            ...
            integers = [discriminator_as_int, amount, collateral_amount, slippage_bps]
    ...
    def sell(
        ...
        try:
            ...
            collateral_amount = get_collateral_amount_by_tokens(
                mint_str, token_balance, TradeDirection.SELL
            )
            amount = int(token_balance * LAMPORTS_PER_SOL)

            print(
                f"Collateral Amount: {collateral_amount}, Amount (in lamports): {amount}, Slippage (bps): {slippage_bps}"
            )
